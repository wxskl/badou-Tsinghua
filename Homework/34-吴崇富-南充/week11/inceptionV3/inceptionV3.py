...
def conv2d_bn(x,filters,num_row,num_col,strides=(1,1),padding='same',name=None): # filters是输出通道数
    if name is not None:
        bn_name = name + '_bn'
...
def InceptionV3(input_shape=[299,299,3],classes=1000):
    ...
    x = conv2d_bn(x,192,3,3,padding='valid') # 感觉这里步长应该为2
    x = MaxPooling2D((3,3),strides=(2,2))(x) # 这一行代码可能有问题，感觉应该舍掉
    ...
    x = layers.concatenate([branch1x1,branch5x5,branch3x3dbl,branch_pool],
                           axis=3,name='mixed1') # layer顺序为(N,H，W,C)时,3是C维度。一定要非常注意，不同框架下layout的顺序不一样。
    ...
    # 平均池化后全连接。
    # 8 x 8 x 2048 -> 1 x 1 x 2048
    x = GlobalAveragePooling2D(name='avg_pool')(x)
    # 1 x 1 x 2048 -> 1 x 1 x 1000
...
def preprocess_input(x): # 转化为[-1,1]之间
    x /= 255.
    x -= 0.5
    x *= 2.
    return x
...
if __name__ == '__main__':
    ...
    x = np.expand_dims(x,axis=0) # 扩充维度N
